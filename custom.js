  ...
  <div class="menu__links">
      ...
      <div class="menu__link">
        ...
        if (action == 'passwordchange') {
            var changePasswordtext = `<div class="change-password">
            ...
            $(".navbar-header").hide();
            $(".alert").hide();
            $("#passwordchangeform").parent().removeAttr("class");
            $("#passwordchangeform").parent().parent().removeAttr("class");
            $("#passwordchangeform").parent().parent().addClass("form-wrapper");
            $(".form-wrapper").find(".panel-heading").hide();
            $("#passwordchangeform").addClass("form-group");
            $("#passwordchangeform").prepend(changePasswordtext);
            $(".btn-cnfrm").parent().addClass("form-action-group");
            $("#confirmPassword").parent().parent().css({
                "margin-top": "10px"
            })
  ...
  Email Address</p>`;
            ...
            $('form').find('input').each(function() {
                if ($(this).prop('required')) {
                    $(this).parent().find("p").addClass("form-label").append("<small>Required </small>");
                }
            });
        ...
        $("#emailDiv").css({ "margin-top": "15px" });
        var link = window.location.href;
        var links = link.split('moas')
        if ($("#email").val() != '' && action == "/moas/newusersignup") {
            $("#email").attr("readonly", true);

            if ($("#email").val().indexOf("privaterelay.appleid") >= 0) {
                $("#row1Div").hide();
            }
        }
        if (links[1] == '/newusersignup') {
            if (errorMessage != "User already exists with this email.") {
                $("#email").removeAttr("readonly");
            }
            if (errorMessage == "Password and confirm password must be same and cannot be blank.") {
                $("#email").attr("readonly", true);
            }
        }
...
<div class="footer__inner">
  <div class="footer__content">
    ...
    $("input[type=text]").addClass("input-custom");
    $("body").append(footer);
    $("body").show();
});